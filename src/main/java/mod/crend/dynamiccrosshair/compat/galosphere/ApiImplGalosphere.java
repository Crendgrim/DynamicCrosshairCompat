...
public class ApiImplGalosphere implements DynamicCrosshairApi {
	...
	public boolean isInteractableBlock(BlockState blockState) {
		Block block = blockState.getBlock();
		return block instanceof AuraRingerBlock || block instanceof WarpedAnchorBlock;
	}
	...
	public Crosshair computeFromBlock(CrosshairContext context) {
		...
		if (block instanceof AuraRingerBlock) {
			if (itemStack.isOf(GBlocks.ALLURITE_BLOCK.asItem()) && !blockState.get(AuraRingerBlock.RINGING)) {
				return Crosshair.USABLE;
			}
		}
	...
	public boolean isInteractableEntity(Entity entity) {
		return entity instanceof SparkleEntity || entity instanceof SpectreEntity;
	}
	...
	public Crosshair computeFromEntity(CrosshairContext context) {
		...
		if (entity instanceof SparkleEntity sparkle) {
			if (sparkle.getCrystaltype() != SparkleEntity.CrystalType.NONE && itemStack.getItem() instanceof PickaxeItem && !sparkle.isBaby()) {
				return Crosshair.USABLE;
			} else if (sparkle.getCrystaltype() == SparkleEntity.CrystalType.NONE && itemStack.isIn(GItemTags.SPARKLE_TEMPT_ITEMS)) {
				return Crosshair.USABLE;
			}
		}

		if (entity instanceof SpectreEntity spectre) {
