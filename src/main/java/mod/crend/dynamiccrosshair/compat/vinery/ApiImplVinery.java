...
public class ApiImplVinery implements DynamicCrosshairApi {
	...
	public boolean isAlwaysInteractableBlock(BlockState blockState) {
		...
		return block instanceof CookingPotBlock
				...
				|| block instanceof PieBlock
				|| block instanceof WinePressBlock
				|| block instanceof WineRackStorageBlock
				|| block instanceof WoodFiredOvenBlock
		;
	}
	...
	public Crosshair computeFromBlock(CrosshairContext context) {
		...
		if (block instanceof TomatoCropBlock) {
			return fromBush(blockState, itemStack, TomatoCropBlock.AGE, 5);
		}
		...
		if (block instanceof BasketBlock) {
			if (context.player.isSneaking() && blockState.get(BasketBlock.STAGE) > 0) {
				return Crosshair.INTERACTABLE;
			} else if (itemStack.isOf(ObjectRegistry.RED_GRAPE.asItem()) && blockState.get(BasketBlock.STAGE) < 1) {
				return Crosshair.USABLE;
			}
		}

		if (block instanceof BreadBlock) {
			if (itemStack.isIn(VineryTags.JAMS) || context.player.canConsume(false)) {
				return Crosshair.USABLE;
			}
		}
		...
		if (block instanceof ChenetBlock chenetBlock) {
			if (itemStack.getItem() == chenetBlock.asItem() && blockState.get(ChenetBlock.STACK) < 2) {
				return Crosshair.USABLE;
			}
		}
		...
		if (block instanceof GrapevinePotBlock potBlock) {
			...
			} else if (itemStack.isOf(ObjectRegistry.WINE_BOTTLE.asItem()) && ((GrapevinePotBlockAccessor) potBlock).invokeCanTakeWine(blockState, itemStack)) {
				return Crosshair.USABLE;
			}
		}

		if (block instanceof KitchenSinkBlock) {
			if (itemStack.isOf(ObjectRegistry.FAUCET) && !blockState.get(KitchenSinkBlock.HAS_FAUCET)) {
				return Crosshair.USABLE;
			} else if (itemStack.isEmpty() && blockState.get(KitchenSinkBlock.HAS_FAUCET) && !blockState.get(KitchenSinkBlock.FILLED)) {
				return Crosshair.INTERACTABLE;
		...
		if (block instanceof StackableBlock) {
			if (itemStack.isOf(block.asItem())) {
				if (blockState.get(StackableBlock.STACK) < 3) {
					return Crosshair.USABLE;
				}
		...
		if (block instanceof WineBottleBlock && context.getBlockEntity() instanceof WineBottleBlockEntity wineEntity) {
			if (itemStack.isOf(block.asItem())) {
				if (!wineEntity.isFull()) {
					return Crosshair.USABLE;
				}
			} else if (itemStack.isEmpty()) {
				if (wineEntity.getCount() >= 1) {
					return Crosshair.INTERACTABLE;
				}
			}
		}
